...
Веб-дашборд для мониторинга и анализа показателей ТОИР на основе 7 Excel-отчетов.

Текущая целевая логика проекта:
1. В `data/` лежат 7 исходных отчетов Excel.
...
- Графики по вкладкам:
  ...
  - `Надежность`: причины отказов, тренд КТГ, топ СННО (MTBF), топ СВВ (MTTR).
...
- `npm run build:dashboard:json` — собрать/обновить `data/toir.json` из 7 Excel-отчетов.
